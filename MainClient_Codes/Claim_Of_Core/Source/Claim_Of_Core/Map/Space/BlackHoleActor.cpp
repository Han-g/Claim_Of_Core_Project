...
void ABlackHoleActor::BeginPlay()
{
	...
	UE_LOG(
		LogTemp,
		Warning,
		TEXT("[BlackHoleActor] BeginPlay / Active: %s / PullRadius: %.1f / PullStrength: %.1f"),
...
void ABlackHoleActor::ConfigureBlackHole(float InPullStrength, float InPullRadius, float InMinDistance, float InMaxPullSpeed, bool bInDrawDebug, FVector InScale)
{
	PullStrength = InPullStrength;
	PullRadius = InPullRadius;
