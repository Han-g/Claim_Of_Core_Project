...
enum PacketID {
    ...
    PKT_S2C_MAP_SELECT_NOTICE = 3,     // 시스템 알림: 맵 선택 동기화
    PKT_S2C_SYNC_GAME_TIME_NOTICE = 4, // 서버 타이머 상시 동기화 (고빈도 호출)
    ...
    PKT_S2C_ROOM_INFO_BRD = 103,            // 방 내부 접속 인원 및 정보 동기화
    PKT_S2C_CHARACTER_SELECT_BRD = 104,     // 다른 유저의 캐릭터(직업) 선택 결과 브로드캐스트
    // 105번은 아래 Event Trigger Packet에서 사용

    PKT_S2C_GAME_START_BRD = 106,           // 방장의 시작 요청에 대한 서버의 게임 시작 명령

    // C2S (500 ~ 999)
    PKT_C2S_ACCESS_REQ = 500,
    PKT_C2S_LOGIN_REQ = 501,
    PKT_C2S_ROOM_CREATE_REQ = 507,          // 클라이언트 -> 서버: 방 생성 요청
    PKT_C2S_ROOM_JOIN_REQ = 508,            // 클라이언트 -> 서버: 방 입장 요청
    PKT_C2S_MAP_SELECT_REQ = 509,           // 클라이언트 -> 서버: 플레이할 맵 선택
    PKT_C2S_CHARACTER_SELECT_REQ = 510,     // 클라이언트 -> 서버: 직업(Role) 선택 요청
    PKT_C2S_GAME_START_REQ = 511,           // 클라이언트(방장) -> 서버: 게임 시작 요청
    ...
    // Combat Packet
    PKT_C2S_ATTACK_KEYINPUT = 504,          // 공격 키 입력 알림
    ...
    PKT_C2S_ITEMPICKUP_KEYINPUT = 505,      // 아이템 줍기 시도
    PKT_C2S_ITEMDROP_KEYINPUT = 506,        // 장착 중인 아이템 버리기 시도
    PKT_C2S_OBJECT_HIT_REQ = 512,           // 파괴 가능한 오브젝트 타격 시도 (서버에 판정 요청)
