...
AMyCharacter::AMyCharacter()
{
	...
	VinePromptTextComponent = CreateDefaultSubobject<UTextRenderComponent>(TEXT("VinePromptText"));
	VinePromptTextComponent->SetupAttachment(GetMesh());
	VinePromptTextComponent->SetRelativeLocation(FVector(0.f, 0.f, 320.f));
	VinePromptTextComponent->SetRelativeRotation(FRotator(0.f, 180.f, 0.f));
	VinePromptTextComponent->SetHorizontalAlignment(EHTA_Center);
	VinePromptTextComponent->SetVerticalAlignment(EVRTA_TextCenter);
	VinePromptTextComponent->SetWorldSize(26.f);
	VinePromptTextComponent->SetText(FText::FromString(TEXT("[E:덩굴타기]")));
	VinePromptTextComponent->SetVisibility(false);
	VinePromptTextComponent->SetHiddenInGame(true);
	VinePromptTextComponent->SetOnlyOwnerSee(true);
	VinePromptTextComponent->SetCastShadow(false);
...
void AMyCharacter::Tick(float DeltaTime)
{
	...
	if (IsLocallyControlled() && bIsVineClimbing)
	{
...
void AMyCharacter::ExitPoisonFog(AActor* FogActor)
{
	...
	PoisonFogVisionSources.RemoveAll([FogActor](const TWeakObjectPtr<AActor>& Source)
	{
		return Source.Get() == FogActor;
	});

	const bool bShouldRemainActive = PoisonFogVisionSources.Num() > 0;
	if (bPoisonFogVisionEffectActive != bShouldRemainActive)
	{
		bPoisonFogVisionEffectActive = bShouldRemainActive;
		UpdateLocalPostProcessEffects();
	}
}
...
{
	PoisonFogVisionSources.RemoveAll([](const TWeakObjectPtr<AActor>& Source)
	{
...
void AMyCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindKey(EKeys::E, IE_Pressed, this, &AMyCharacter::VineInteractPressed);
...
void AMyCharacter::UpdateVineClimbMovement()
{
	...
	if (UCharacterMovementComponent* MoveComp = GetCharacterMovement())
	{
		...
		const float ClimbAxis = FMath::Abs(CachedMoveForward) >= VineClimbInputThreshold
			? CachedMoveForward
			: 0.f;

		FVector ClimbUpDirection = VineActor->GetActorUpVector();
		ClimbUpDirection = ClimbUpDirection.GetSafeNormal();

		MoveComp->Velocity = ClimbUpDirection * ClimbAxis * VineClimbSpeed;
	}
}
...
void AMyCharacter::JumpOffVine()
{
	...
	if (VineActor)
	{
		JumpDirection = VineActor->GetActorForwardVector();
	}
...
void AMyCharacter::UpdateVinePromptText()
{
	if (!VinePromptTextComponent)
	{
		return;
	}
	...
	VinePromptTextComponent->SetVisibility(bShouldShow);
	VinePromptTextComponent->SetHiddenInGame(!bShouldShow);
}
...
void AMyCharacter::UpdateLocalPostProcessEffects()
{
	...
	FollowCamera->PostProcessSettings.bOverride_DepthOfFieldDepthBlurAmount = false;
	FollowCamera->PostProcessSettings.bOverride_DepthOfFieldScale = false;
	...
	if (bPoisonFogVisionEffectActive)
	{
		const float Saturation = bLowHPEffectActive
			? FMath::Min(LowHPSaturation, PoisonFogSaturation)
			: PoisonFogSaturation;

		const float Vignette = bLowHPEffectActive
			? FMath::Max(LowHPVignetteIntensity, PoisonFogVignetteIntensity)
			: PoisonFogVignetteIntensity;

		const FLinearColor SceneTint = bLowHPEffectActive
			? (LowHPSceneTint * PoisonFogSceneTint)
			: PoisonFogSceneTint;
		...
		FollowCamera->PostProcessSettings.bOverride_DepthOfFieldDepthBlurAmount = true;
		...
		FollowCamera->PostProcessSettings.ColorSaturation = FVector4(Saturation, Saturation, Saturation, 1.f);
		FollowCamera->PostProcessSettings.VignetteIntensity = Vignette;
		FollowCamera->PostProcessSettings.SceneColorTint = SceneTint;
		FollowCamera->PostProcessSettings.SceneFringeIntensity = PoisonFogSceneFringeIntensity;
		FollowCamera->PostProcessSettings.MotionBlurAmount = PoisonFogMotionBlurAmount;
		FollowCamera->PostProcessSettings.DepthOfFieldFocalDistance = PoisonFogDepthOfFieldFocalDistance;
		FollowCamera->PostProcessSettings.DepthOfFieldDepthBlurAmount = PoisonFogDepthBlurAmount;
		FollowCamera->PostProcessSettings.DepthOfFieldScale = PoisonFogDepthOfFieldScale;
		return;
	}
	...
	FollowCamera->PostProcessSettings.SceneColorTint = FLinearColor::White;
	FollowCamera->PostProcessSettings.SceneFringeIntensity = 0.f;
	FollowCamera->PostProcessSettings.MotionBlurAmount = 0.f;
	FollowCamera->PostProcessSettings.DepthOfFieldFocalDistance = 0.f;
	FollowCamera->PostProcessSettings.DepthOfFieldDepthBlurAmount = 1.f;
	FollowCamera->PostProcessSettings.DepthOfFieldScale = 0.f;
}
